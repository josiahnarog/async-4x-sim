...
2. **Combat features** *(in progress)* — Fog of war, sensor suites, carrier Bl enforcement, AI opponents (move AI with approach-bonus scoring, squadron AI with greedy launch), mouse-driven UI (click-to-select, right-click commands, left→right→left move gesture with BFS reachability), carrier refuel/rearm (docked squadrons restore endurance and ordnance each turn), mount type system (spinal/broadside arc enforcement and annotations), integration test (full AI-vs-AI battle loop) all done. **Next: retreat/withdrawal mechanic** (resolve stalemates; preserve crippled ships). Then: expanded scenarios for playtesting.
